    ...
    (dispatch) =>
        bindActionCreators(
            Object.keys(actions)
                .reduce((res, key) => {
                    res[key] = (...args) =>
                        (actions[key]._customAC === true ? actions[key](...args) :
...
export const createCustomCreator = (func) => {
    const actionCreator = (...args) =>
        (dispatch, getState) =>
            func(...args, dispatch, getState, createDispatcher(dispatch));
 ...
 *      })
 */
const createBoundActionCreators = (types, overrides) =>
    getBoundActionCreators(getActionsSettings(types, overrides));

export default createBoundActionCreators;
